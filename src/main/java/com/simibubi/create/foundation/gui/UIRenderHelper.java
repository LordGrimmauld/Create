...
public class UIRenderHelper {
	...
	public static void init() {
		RenderSystem.recordRenderCall(() -> {
			MainWindow mainWindow = Minecraft.getInstance()
					.getWindow();
			framebuffer = new Framebuffer(mainWindow.getWidth(), mainWindow.getHeight(), true,
					Minecraft.ON_OSX);
	...
	public static void setupSimpleCustomLighting(float xRot, float yRot) {
		...
		lightingMatrix.setIdentity();
		lightingMatrix.multiply(Vector3f.YP.rotationDegrees(yRot));
		lightingMatrix.multiply(Vector3f.XP.rotationDegrees(xRot));
		lightingMatrix.multiply(Matrix4f.createTranslateMatrix(0, 0, 1));
