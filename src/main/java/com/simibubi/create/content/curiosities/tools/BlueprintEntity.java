...
public class BlueprintEntity extends HangingEntity
	...
	public boolean survives() {
		...
		Direction direction = this.direction.getAxis()
			.isVertical() ? verticalOrientation.getClockWise() : this.direction.getCounterClockWise();
	...
	public ActionResultType interactAt(PlayerEntity player, Vector3d vec, Hand hand) {
		...
		if (!AllItems.WRENCH.isIn(player.getItemInHand(hand)) && !level.isClientSide) {
			boolean empty = true;
