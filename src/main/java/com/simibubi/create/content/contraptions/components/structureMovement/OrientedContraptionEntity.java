...
public class OrientedContraptionEntity extends AbstractContraptionEntity {
	...
	protected boolean updateOrientation(boolean rotationLock, boolean wasStalled, Entity riding, boolean isOnCoupling) {
		...
		if (!isInitialOrientationPresent() && !level.isClientSide) {
			if (locationDiff.length() > 0) {
				Direction facingFromVector =
					Direction.getNearest(locationDiff.x, locationDiff.y, locationDiff.z);
				if (initialYawOffset != -1)
					facingFromVector = Direction.fromYRot(facingFromVector.toYRot() - initialYawOffset);
