...
public class CombinedFluidHandler implements IFluidHandler {
    ...
    public boolean isFluidValid(int tank, @Nonnull FluidStack stack) {
        return (!stack.isEmpty()) && (tanks[tank].isEmpty() || tanks[tank].isFluidEqual(stack)) && tanks[tank].getAmount() < capacity;
    }
    ...
    private boolean hasFluid(Fluid fluid) {
        return Arrays.stream(tanks).anyMatch(fluidStack -> fluidStack.getFluid() == fluid);
    }

    private int searchFluidSlot(Fluid fluid) {
        return IntStream.range(0, tanks.length).filter(i -> tanks[i].getFluid() == fluid && !tanks[i].isEmpty()).findFirst().orElse(-1);
    }

    private int getFittingFluidSlot(FluidStack fluidStack) {
        return IntStream.range(0, tanks.length).filter(i -> isFluidValid(i, fluidStack)).findFirst().orElse(-1);
    }
    ...
    public ListNBT getListNBT() {
        return Arrays.stream(tanks).map(fluid -> fluid.writeToNBT(new CompoundNBT())).collect(Collectors.toCollection(ListNBT::new));
    }

    public void forEachTank(NonNullConsumer<FluidStack> fluidStackConsumer) {
        Arrays.stream(tanks).forEach(fluidStackConsumer::accept);
    }
}
